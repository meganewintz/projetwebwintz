<!DOCTYPE HTML>
<html>
	<head>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />

    <!-- CSS stylesheets -->
		<link rel="stylesheet" href="/public/templated/assets/css/main.css" />
    <title>My Appli</title>
	</head>
	<body>
		<div class="page-wrap">

			<!-- Nav -->
				<nav id="nav">
					<ul>
						<li><a href="/index.php" class="active"><span class="icon fa-home fa-lg"></span></a></li>
						<li><a href="/app/views/galerie.php"><span class="icon fa-camera-retro fa-lg"></span></a></li>
						<li><a href="/app/views/ajoutPhoto.php"><span class="fa fa-cloud-upload fa-lg"></a></li>
						<li><a href="/app/views/aboutMe.php"><span class="icon fa-file-text-o"></span></a></li>
					</ul>
				</nav>

			<section id="main">
